...
本期不包含附件/佐证材料、外部消息推送、强制分布、申诉、周期自动归档和跨企业人员调动的历史考核处理。
...
- 员工以工号为唯一标识，必须归属一个子公司，并保存姓名、部门、职位、职级、手机号和在职状态。职级固定为员工、副总经理、总经理，与自由填写的职位不同。
...
发布周期时，系统对**所有在职员工**统一校验：每人必须已有考核单、至少一名有效打分人、合计恰为 100.00% 的打分权重，并有一棵完整且合法的指标树。任一员工不满足条件，发布失败并返回最多十名问题员工及原因。
...
- 每条打分关系都包含权重，单人固定为 100.00%。管理员可为每个子公司部门设置总经理、副总经理、员工三个正整数系数。按被打分人所属部门的预设，系统将选中的打分人个人系数除以所选人员系数总和，得到百分比；未选中的员工不参与计算。例如系数 3:2:1、选中 1 位总经理、2 位副总经理、3 位员工时，各人权重依次为 30%、20%、20%、10%、10%、10%。无预设时系统均分，管理员始终可以手动调整。权重须大于 0、最多两位小数且合计恰为 100.00%。
- 预设或职级修改只影响后续应用，不追改已保存关系；草稿期管理员可主动重新应用预设。计算时以工号作为稳定的尾差分配顺序，确保总和为 100.00%。
- 周期发布后打分关系与权重冻结；管理员审核通过时，系统将权重写入每一条评分任务快照。后续计算与导出只以该快照为准。
...
- 当一份考核单的全部评分任务均已提交，考核单进入 `COMPLETED`，最终总分为 `Σ(打分人总分 × 评分任务权重 ÷ 100)`，仅最终结果保留两位小数。
- 停止周期后，任何评分保存或提交均被锁定。

## 7. 导入、导出与提醒

- 管理员可下载员工、指标和打分关系 Excel 模板；员工模板包含“职级”列。打分关系模板包含“权重（%）”列，同一员工的全部权重列留空时有部门预设则按职级计算，否则均分；支持 `.xlsx`。
...
- 超级管理员可导出评分明细，其中包含打分人、权重、打分人总分、加权贡献、叶子指标得分与评语。
...
5. 全部打分人提交后，系统按冻结权重自动生成最终总分；管理员只能在其授权范围全部完成后导出最终结果。
