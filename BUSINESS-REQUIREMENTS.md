...
- 每条打分关系都包含权重，单人固定为 100.00%；多人权重由系统初次均分，管理员可调整。每位员工在一个周期内的全部打分人权重必须大于 0、最多两位小数且合计恰为 100.00%。
...
- 管理员可下载员工、指标和打分关系 Excel 模板；打分关系模板包含“权重（%）”列，若同一员工的全部权重列留空则系统均分；支持 `.xlsx`。
