...
export async function configureAssessment(actor: Actor, input: { cycleId: string; employeeNo: string; nodes: NewIndicator[]; scorers: string[] }) {
  ...
  if (input.scorers.length === 0 || new Set(input.scorers).size !== input.scorers.length || input.scorers.includes(input.employeeNo)) {
    throw new HttpError(422, "INVALID_SCORERS", "至少配置一名不同于本人的打分人，且不可重复");
  }
  return inTransaction(async (client) => {
    ...
    if (!cycle.rows[0] || cycle.rows[0].companyId !== target.rows[0].companyId || cycle.rows[0].status !== "DRAFT") {
      throw new HttpError(409, "CYCLE_NOT_DRAFT", "仅草稿周期可配置指标");
    }
    const scorers = await client.query("select e.employee_no as \"employeeNo\", e.subsidiary_id as \"subsidiaryId\", s.company_id as \"companyId\" from employees e join subsidiaries s on s.id=e.subsidiary_id where e.employee_no=any($1::text[]) and e.status='ACTIVE'", [input.scorers]);
    ...
    if (!assessmentId) await client.query("insert into assessments (id,cycle_id,employee_no) values ($1,$2,$3)", [id, input.cycleId, input.employeeNo]);
    else {
      await client.query("delete from indicator_nodes where assessment_id=$1", [id]);
      await client.query("delete from scorer_assignments where cycle_id=$1 and employee_no=$2", [input.cycleId, input.employeeNo]);
    }
...
export async function publishCycle(actor: Actor, cycleId: string, key: string | null) {
  ...
  return withIdempotency(actor, key, { cycleId, action: "PUBLISH" }, async (client) => {
    ...
    const missing = await client.query(`select e.employee_no from employees e join subsidiaries s on s.id=e.subsidiary_id
      left join assessments a on a.employee_no=e.employee_no and a.cycle_id=$1
      where s.company_id=$2 and e.status='ACTIVE' and a.id is null limit 10`, [cycleId, cycle.rows[0].companyId]);
    if (missing.rows.length) throw new HttpError(422, "MISSING_ASSESSMENTS", `以下员工尚未配置指标：${missing.rows.map((row) => row.employee_no).join("、")}`);
    const noScorers = await client.query(`select e.employee_no from employees e join subsidiaries s on s.id=e.subsidiary_id
      where s.company_id=$1 and e.status='ACTIVE' and not exists
      (select 1 from scorer_assignments sa where sa.cycle_id=$2 and sa.employee_no=e.employee_no and sa.status='ACTIVE') limit 10`, [cycle.rows[0].companyId, cycleId]);
...
export async function saveSelf(actor: Actor, assessmentId: string, expectedVersion: number, values: { nodeId: string; content: string }[]) {
  return inTransaction(async (client) => {
    ...
    if (assessment.cycleStatus !== "ACTIVE" || !["DRAFT", "RETURNED"].includes(assessment.status)) throw new HttpError(409, "LOCKED", "当前考核单不可编辑");
...
export async function submitSelf(actor: Actor, assessmentId: string, expectedVersion: number, key: string | null) {
  return withIdempotency(actor, key, { assessmentId, expectedVersion, action: "SUBMIT_SELF" }, async (client) => {
    ...
    if (assessment.cycleStatus !== "ACTIVE" || !["DRAFT", "RETURNED"].includes(assessment.status)) throw new HttpError(409, "LOCKED", "当前考核单不可提交");
    assertVersion(assessment.version, expectedVersion);
    const missing = await client.query(`select n.name from indicator_nodes n where n.assessment_id=$1 and n.id not in
      (select parent_id from indicator_nodes where assessment_id=$1 and parent_id is not null) and trim(n.self_content)=''`, [assessmentId]);
    if (missing.rows.length) throw new HttpError(422, "INCOMPLETE", `以下指标尚未填写：${missing.rows.map((row) => row.name).join("、")}`);
...
export async function approveAssessment(actor: Actor, assessmentId: string, expectedVersion: number, key: string | null) {
  return withIdempotency(actor, key, { assessmentId, expectedVersion, action: "APPROVE" }, async (client) => {
    const assessment = await assessmentForUpdate(client, assessmentId);
    assertAdmin(actor, assessment.subsidiaryId);
    if (assessment.status !== "SUBMITTED" || assessment.cycleStatus !== "ACTIVE") throw new HttpError(409, "NOT_SUBMITTED", "只能通过待预审考核单");
    assertVersion(assessment.version, expectedVersion);
...
export async function saveScore(actor: Actor, taskId: string, expectedVersion: number, items: { nodeId: string; score: number; comment: string }[]) {
  return inTransaction(async (client) => {
    ...
    if (task.status !== "PENDING" || task.assessmentStatus !== "SCORING" || task.cycleStatus !== "ACTIVE") throw new HttpError(409, "LOCKED", "打分任务已锁定");
