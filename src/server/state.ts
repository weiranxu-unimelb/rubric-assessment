...
export async function loadState(actor: Actor) {
  const [companies, subsidiaries, cycles, mine, myNodes, myTasks] = await Promise.all([
    ...
    pool.query("select id, company_id as \"companyId\", name from subsidiaries where $1::boolean or id=$2 or id=any($3::text[]) order by name", [actor.isSuperAdmin, actor.subsidiaryId, actor.adminSubsidiaryIds]),
    pool.query("select id, company_id as \"companyId\", name, status, starts_at as \"startsAt\", ends_at as \"endsAt\" from cycles where $1::boolean or company_id in (select company_id from subsidiaries where id=$2 or id=any($3::text[])) order by created_at desc", [actor.isSuperAdmin, actor.subsidiaryId, actor.adminSubsidiaryIds]),
  ...
  if (scope.length > 0) {
    const [employees, assessments, nodes, assignments, progress] = await Promise.all([
      pool.query("select employee_no as \"employeeNo\", subsidiary_id as \"subsidiaryId\", name, position, phone, status, is_super_admin as \"isSuperAdmin\" from employees where subsidiary_id=any($1::text[]) order by name, employee_no", [scope]),
    ...
    adminData = { employees: employees.rows, assessments: assessments.rows, nodes: nodes.rows, assignments: assignments.rows, progress: progress.rows };
  }
  ...
  if (actor.isSuperAdmin) {
    const [details, adminScopes] = await Promise.all([
      ...
      pool.query("select admin_employee_no as \"adminEmployeeNo\", subsidiary_id as \"subsidiaryId\" from admin_scopes"),
    ]);
