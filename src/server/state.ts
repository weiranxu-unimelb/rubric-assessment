...
export async function loadState(actor: Actor) {
  const [companies, subsidiaries, cycles, mine, myNodes, myTasks, templates] = await Promise.all([
    ...
    pool.query("select t.id, t.assessment_id as \"assessmentId\", t.status, t.version, t.total_score as \"totalScore\", t.weight, a.employee_no as \"employeeNo\", e.name as \"employeeName\", a.cycle_id as \"cycleId\" from score_tasks t join assessments a on a.id=t.assessment_id join employees e on e.employee_no=a.employee_no where t.scorer_employee_no=$1 order by t.id", [actor.employeeNo]),
